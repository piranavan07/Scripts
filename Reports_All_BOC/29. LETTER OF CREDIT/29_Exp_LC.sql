CREATE OR REPLACE VIEW 29_Exp_LC
AS
  SELECT
    ...
    OUTSTANDING_AMOUNT,
    CAST( (MAS.AMOUNT /POWER(10,C8PF.C8CED))*NVL(SPOT.SPOTRATE,1) AS DECIMAL(20
    ,0)) AS AMOUNT_IN_BASE_CURRENCY,
    CAST((MAS.AMT_O_S/POWER(10,C8PF.C8CED))*NVL(SPOT.SPOTRATE,1) AS DECIMAL(20,
    0))           AS OS_AMT_IN_BASE,
    PAD1.ADDRESS1 AS COUNTER_PARTY,
    PAD1.COUNTRY  AS COUNTER_PARTY_COUNTRY,
	NULL          AS COUNTER_PARTY_COUNTRY_SWIFT,----------------TODO
	NULL		  AS NOSORT_BANK,----------------TODO
    ...
    AVAILABILITY,----------------TODO
	NULL 		 AS  WITH_WITHOUT_CONFIRMATION_INSTRUCTIONS, --------------TODO
    DECODE(LCM.REVOLVING,'Y','Yes','N','No') AS REVOLVING,----------------TODO
    CASE PRD.SHORTN13
      WHEN 'Back/Back'
      THEN 'YES'
      ELSE 'NO'
...
--    CAST(NVL(CHG.SWIFT_CHG,0) AS  DECIMAL(20,0)) AS SWIFT_CHG,
      ...
      FROM
        (
          SELECT
            ...
            CASE
              WHEN RLT.DESCR LIKE '%Swift%'----------------TODO
  ...
  WHERE PRD.CODE79     = 'SHG' --SHIPPING GUARANTEE----------------TODO
